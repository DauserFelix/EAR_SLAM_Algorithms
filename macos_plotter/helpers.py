...
### irgend ne se2 alignment von chatgpt###

def align_se2_xy(ref_xy, algo_xy):
    """
    Findet beste 2D-Rotation+Translation, die algo_xy auf ref_xy legt (least squares).
    ref_xy, algo_xy: shape (N,2)
    return: algo_xy_aligned (N,2)
    """
    # Zentrieren
    ref_mean = ref_xy.mean(axis=0)
    algo_mean = algo_xy.mean(axis=0)

    X = ref_xy - ref_mean
    Y = algo_xy - algo_mean

    # Beste Rotation via SVD
    H = Y.T @ X
    ...
    # Translation
    t = ref_mean - (R @ algo_mean)

    # Anwenden
    algo_aligned = (R @ algo_xy.T).T + t
    return algo_aligned



def compute_errors(ref, interp):
    # --- Position (Startpunkt-Alignment in XY) ---
    ref_xy = np.c_[ref["px"].values, ref["py"].values]
    algo_xy = np.c_[interp["px"], interp["py"]]

    algo_xy_aligned = align_se2_xy(ref_xy, algo_xy)

    err_xy = np.linalg.norm(algo_xy_aligned - ref_xy, axis=1)
...
# ============================================================
def make_output_folder(mode, base_dir=None):
    if base_dir is None:
...
# MENÜ
# ============================================================
def list_csv_files(data_dir=None):
    if data_dir is None:
