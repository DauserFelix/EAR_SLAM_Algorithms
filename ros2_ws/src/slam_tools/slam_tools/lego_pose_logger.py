#!/usr/bin/env python3
"""
Dieses Skript loggt die Odometry-Ausgaben von LeGO-LOAM in eine CSV-Datei.
Es abonniert das Topic /aft_mapped_to_init (nav_msgs/Odometry) und
...
Zweck:
Erzeugt eine vollständige Zeitreihe der LeGO-LOAM-Pose für spätere
...
class PoseLogger(Node):
    def __init__(self):
        super().__init__("pose_logger_lego")

        # Timestamped filename
        timestamp = datetime.now().strftime("%m-%d_%H-%M")
        self.csv_path = f"/data/{timestamp}_lego_pose_logger_output.csv"
        ...
        # Subscribe to LeGO-LOAM odometry
